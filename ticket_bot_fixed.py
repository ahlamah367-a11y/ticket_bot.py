...
async def apply_private_ticket_permissions(channel, ticket):
    """عند تفعيل الوضع الخاص: صاحب التذكرة + الإداري المستلم فقط."""
    ...
    if not private_mode or not claimed_id:
        return

    overwrites = {}
    guild = channel.guild

    # منع الرؤية العامة. الإداري صاحب صلاحية Administrator يظل Discord نفسه يسمح له بتجاوز الـ overwrite.
...
def check_staff(interaction):
    ...
    # في الوضع الخاص لا يتدخل في أوامر الإدارة إلا الإداري الذي استلم التذكرة.
    # صاحب التذكرة نفسه تتم معالجة صلاحية أزرار الإغلاق/التقييم له بشكل منفصل.
...
async def ticket_mode(
    ...
    # تطبيق الإعداد فوراً على التذاكر المفتوحة من نفس النوع والمستلمة بالفعل.
    for channel_id, ticket in list(database.get("open_tickets", {}).items()):
        if ticket.get("type") != ticket_id or not ticket.get("claimed"):
            continue
    ...
    await interaction.response.send_message(
        ...
        "في الخاص: بعد الاستلام تكون التذكرة لصاحبها والإداري المستلم.",
        ephemeral=True
    )
