...
if "log_channels" not in database:
    database["log_channels"] = {"ticket": None}
elif "ticket" not in database["log_channels"]:
    database["log_channels"]["ticket"] = None
...
async def send_ticket_log(guild, title, description="", color=discord.Color.blue(), fields=None, file_path=None):
    log_id = database.get("log_channels", {}).get("ticket")
    if not log_id:
        return
    ...
    embed.set_footer(text="🎫 Ticket Logs • Unified Log")
    try:
        await log.send(embed=embed, file=discord.File(file_path) if file_path else None)
    except Exception as e:
        print(f"Ticket log error: {e}")


async def log_ticket_action(interaction, action, details="", color=discord.Color.blue()):
    ticket = get_ticket_from_channel(interaction.channel.id)
    if not ticket:
        return
    settings = database["tickets"].get(ticket.get("type"))
    await send_ticket_log(
        interaction.guild, f"📝 {action}", fields=[
            ...
            ("📅 الوقت", f"<t:{int(datetime.now().timestamp())}:F>", False),
        ], color=color
    )
...
async def add_ticket_panel(
    ...
    save_database()
    await send_ticket_log(interaction.guild, "🖥️ إنشاء بانل", fields=[
...
async def delete_ticket_panel(
    ...
    save_database()
    await send_ticket_log(interaction.guild, "🗑️ حذف بانل", fields=[
        ("🖥️ البانل", panel_id, True), ("👤 بواسطة", interaction.user.mention, True)
...
async def panel_add_ticket(
    ...
    save_database()
    await send_ticket_log(interaction.guild, "➕ إضافة نوع تذكرة إلى بانل", fields=[
...
async def panel_remove_ticket(
    ...
    panel["tickets"].remove(ticket_id)
    save_database()
    await send_ticket_log(interaction.guild, "➖ إزالة نوع تذكرة من بانل", fields=[
...
async def send_open_log(interaction, channel, ticket):
    ...
    await send_ticket_log(interaction.guild, "🎫 فتح تذكرة", fields=[
...
async def send_claim_log(interaction, channel, ticket):
    ...
    await send_ticket_log(interaction.guild, "👑 استلام تذكرة", fields=[
...
async def send_close_log(interaction, channel, transcript=None, rating=None, rating_reason=None):
    ...
    await send_ticket_log(interaction.guild, "🔒 إغلاق تذكرة", fields=[
        ("👤 صاحب التذكرة", f"<@{ticket.get('owner')}>", True),
        ("🎟️ النوع", settings.get("name", "غير معروف") if settings else "غير معروف", True),
...
async def send_rating_log(interaction, stars, ticket, rating_reason=None):
    staff = ticket.get("claimed")
    await send_ticket_log(interaction.guild, "⭐ تقييم تذكرة", fields=[
        ("👤 صاحب التذكرة", f"<@{ticket.get('owner')}>", True),
...
@bot.tree.command(name="ticket-logs", description="تحديد روم اللوق الموحد لجميع أحداث التذاكر")
@app_commands.describe(channel="روم اللوق الذي ستصل إليه جميع سجلات التذاكر والبانلات")
async def ticket_logs(interaction: discord.Interaction, channel: discord.TextChannel):
    if not interaction.user.guild_permissions.administrator:
        await interaction.response.send_message("❌ هذا الأمر مخصص للمشرفين فقط", ephemeral=True)
        return
    database.setdefault("log_channels", {})["ticket"] = channel.id
    save_database()
    await interaction.response.send_message(
        embed=make_embed(
            "📜 تم ضبط اللوق الموحد",
            f"سيتم إرسال **فتح / استلام / إغلاق / تقييم / ملاحظات / إجراءات الإدارة** "
            f"لجميع أنواع التذاكر والبانلات إلى {channel.mention}.",
            discord.Color.green()
        ), ephemeral=True
    )
...
async def ticket_note(
    ...
    save_database()
    await log_ticket_action(interaction, "إضافة ملاحظة", f"📝 {note}", discord.Color.gold())
